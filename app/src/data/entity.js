...
for (const [type, map] of Object.entries(ABOUT.details.codes)) {
...
class FileMeta {
    constructor(content, user) {
        ...
        Object.defineProperty(this, "quantize", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
...
class Author {
    constructor(message, user, content, reply = undefined) {
        const valid = (x) => x && ('string' == typeof x || 'number' == typeof x);
...
for (const name in ABOUT.details.types) {
    ...
    const fields = ABOUT.details.types[name];
    const defaults = ABOUT.details.objects[name];
...
for (const enm in ABOUT.details.codes) {
    const map = ABOUT.details.codes[enm];
...
const Entity = Object.seal({
    ...
    do_listmapmap(listmapmap) {
        for (const name in listmapmap)
            this.do_listmap(listmapmap[name]);
        return listmapmap;
    },
    do_listmap(listmap) {
        for (const name in listmap)
    ...
    do_list(list, name) {
        ...
        const type = list.Type || this.key_type(name);
        const cons = TYPES[type] || ((x) => x); // fallback (bad)
    ...
    link_comments({ message, user, content, replies }) {
        ...
        for (const m of message) {
            m.Author = new Author(m, user[~m.createUserId], content?.[~m.contentId], m.values.replyingTo ? replies?.[~m.values.replyingTo] : null);
