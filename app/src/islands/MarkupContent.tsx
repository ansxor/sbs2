...
export interface MarkupContentProps {
  // Source text + markup language, passed straight to `convert_lang`.
  text: string
  lang: string
  // Per-call options forwarded by reference (e.g. `{preview:boolean}`). Keep stable.
  etc?: object | null
  // Container tag + class. Defaults to a plain `<div>`; EditView passes `scroll-inner` with
  // `pageContents editPageContents` so the preview node keeps its exact tag/classes for CSS.
  tag?: string
  className?: string
  // Synchronous hooks around the convert, in this same layout effect (EditView scroll math).
...
export function MarkupContent({
  ...
  return <Tag ref={setNode} className={className} />
}
