...
import type { BaseEntity, Content, SlotHeaderApi } from '../data/types'
import { content_label } from '../services/draw-dom'

// ---- context (frozen L0 exports, signatures unchanged) ----------------------------------
export const SlotHeaderContext = createContext<SlotHeaderApi | null>(null)

export function useSlotHeader(): SlotHeaderApi {
  const header = useContext(SlotHeaderContext)
  if (!header) throw new Error('useSlotHeader must be used inside a Slot')
  return header
}
...
function apply_document_title(title: string): void {
  title = title.replace(/  /g, '  ').replace(/\n/g, '  \n')
  document.title = title
}

// The header nodes the Slot owns — accessed lazily at call time (populated by React commit before
// any effect can invoke the api). Passed as getters to sidestep RefObject invariance between the
// concrete element types (<h1>/<div>/<span>) and HTMLElement.
export interface SlotHeaderRefs {
  title: () => HTMLElement | null
  buttons: () => HTMLElement | null
  extra: () => HTMLElement | null
}

// Superset of the frozen SlotHeaderApi: also exposes the live $header_extra element. editpage.js
// (L7c) appended its Save button to `Slot.$header_extra`; since the frozen SlotHeaderApi (in
// data/types.ts, not owned here) does not include it, views that need it cast the header to this.
export interface SlotHeader extends SlotHeaderApi {
  readonly header_extra: HTMLElement | null
}

export function createSlotHeaderApi(refs: SlotHeaderRefs): SlotHeader {
  return {
    ...
    set_entity_title(entity: BaseEntity): void {
      const name2 = (entity as Content).name2
      apply_document_title(name2)
      const el = refs.title()
      if (el) el.replaceChildren(content_label(entity as Content))
    },
    ...
    add_header_links(...links: Node[]): void {
      ...
      if (el) for (const link of links) el.append(link)
    },
    get header_extra(): HTMLElement | null {
      return refs.extra()
    },
  }
}
