...
let first = true;
export function useViewLifecycle(url, header) {
    ...
    useLayoutEffect(() => {
        ...
        const commit_error = (title, message) => {
        ...
        const do_render = (Component, data) => {
            if (ac.signal.aborted)
                return;
            if (first)
                console.log('\u{1F304} Rendering first page');
        ...
        try {
            ...
            if ('quick' in start) {
                ...
                handle = Lp.chain(chain, (resp, err) => {
                    if (ac.signal.aborted)
                        return;
                    handle = null;
                    if (err) {
