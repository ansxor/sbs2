...
const protectedSlots = new Set<HTMLElement>()

// view.js:147 View.beforeunload
function beforeunload(ev: BeforeUnloadEvent): void {
  if (protectedSlots.size) ev.preventDefault()
}
...
export function protect(slot: HTMLElement, state: boolean): void {
  if (protectedSlots.has(slot) === state) return

  if (state) protectedSlots.add(slot)
  else protectedSlots.delete(slot)

  if (protectedSlots.size) {
    if (!window.onbeforeunload) window.onbeforeunload = beforeunload
  } else {
    if (window.onbeforeunload) window.onbeforeunload = null
  }
}

export function is_protected(slot: HTMLElement): boolean {
...
export function confirm_leave(slot: HTMLElement): boolean {
  if (!protectedSlots.has(slot)) return true
...
window.addEventListener(
  'click',
  (ev) => {
    if (!protectedSlots.size) return
    const el = ev.target
    const link = (el instanceof Element ? el.closest(':any-link') : null) as HTMLAnchorElement | null
    if (!link) return
    if (link.target === 'self') return // in-page anchor scroll — no navigation, no confirm

    let href = link.getAttribute('href') // note: can't use link.href
    if (href && href.startsWith('https://oboy.smilebasicsource.com/12/')) href = link.hash
    if (!href || !href.startsWith('#')) return

    const slotEl = link.closest('view-slot') as HTMLElement | null
    if (!slotEl || !protectedSlots.has(slotEl)) return
