...
// Extra descriptor fields the old SettingProto.draw reads that are not in the shared
// SettingDescriptor shape (only ever set by higher-layer descriptors; none used by the built-ins).
type RawSetting = SettingDescriptor & {
...
type RenderFn = (row: HTMLElement, elem: HTMLElement, label: HTMLElement) => void

// The panel element the code-editor swap parks a row's children in (index.html `$sidebarEditorPanel`),
// carrying a custom `onpause` hook read by focus-nav (Escape → collapse editor → back()).
type EditorPanel = HTMLElement & { onpause?: ((ev?: Event) => void) | null }

export interface SettingsFormProps {
  // Sidebar.tabs.select — code-editor swap switches to the 'editor' / 'user' tabs.
  selectTab?: (name: string) => void
  // Sidebar.output — sitecss `reload css` progress messages ("reloading ‘x’..." → " [OK]"/" [ERROR]").
  output?: (stuff: string) => HTMLElement
}
...
function reloadCssRender(output: SettingsFormProps['output']): RenderFn {
  return (_row, _elem, label) => {
    ...
    btn.onclick = () => {
      for (const sheet of [...document.styleSheets])
        if (sheet.href) {
          const node = sheet.ownerNode as HTMLLinkElement
          const nw = node.cloneNode() as HTMLLinkElement
          const note = output
...
function drawRow(
  field: RawSetting,
  props: { selectTab?: (name: string) => void; output?: (stuff: string) => HTMLElement },
): HTMLElement {
  ...
  const getValue = (): unknown => Settings.get(name)
  ...
  if (type === 'select') {
    ...
    let found = false
    const val = getValue()
    const options = field.options as unknown as string[]
    const labels = field.options_labels || options
    options.forEach((value, i) => {
      const opt = document.createElement('option')
      opt.value = value
      opt.text = labels[i] as string
      select.add(opt)
    ...
    select.value = getValue() as string
    ...
    const back = (): void => {
      ...
      if (panel) row.append(...panel.childNodes)
      in_editor = false
    }
    ...
    btn.onclick = () => {
      if (in_editor) {
        ...
        const panel = document.getElementById('$sidebarEditorPanel') as EditorPanel | null
        if (!panel) return
        if (panel.hasChildNodes()) return // uh oh
  ...
  ;(elem as HTMLInputElement).value = getValue() as string

  row.prepend(elem)
  row.prepend(label)

  ;(elem as HTMLInputElement).onchange = () => {
    // SettingProto.change('change') — value defaults to read() == elem.value.
    Settings.change(name, (elem as HTMLInputElement).value, 'change')
  }
  ...
  if (field.autosave === false) {
    ...
    btn.onclick = () => {
      Settings.change(name, (elem as HTMLInputElement).value, 'update')
    }
    row.append(btn)
  }

  // render augmentation: honor a descriptor-provided render fn, plus the sitecss render deferred here.
  const render: RenderFn | undefined =
    typeof field.render === 'function'
      ? (field.render as RenderFn)
      : name === 'sitecss'
        ? reloadCssRender(props.output)
        : undefined
  if (render) render(row, elem, label)
 ...
 */
export function SettingsForm({ selectTab, output }: SettingsFormProps): React.JSX.Element {
  ...
  useLayoutEffect(() => {
    ...
    if (!host) return
    const fields = Object.values(Settings.fields) as RawSetting[]
  ...
  return <div className="local-settings" ref={ref} />
}
