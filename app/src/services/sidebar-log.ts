...
// A sink consumes one print(...) call's argument list. The Sidebar registers the real
// one (render each arg via sidebar_debug and append to the debug scroller).
export type PrintSink = (args: unknown[]) => void

let sink: PrintSink | null = null
...
export function setPrintSink(fn: PrintSink | null): void {
	sink = fn
	if (fn) {
...
export function format_error(thing: Error): Node {
...
export function sidebar_debug(thing: unknown): Node {
	...
	case 'string': {
		...
		const src = Function.prototype.toString.call(thing as Function)
		e.append(src)
	} break; case 'object': {
		...
		let text = "{???}"
		const pro = Object.getPrototypeOf(thing as object)
