// Pure render helpers ported verbatim from draw.js / apx.js / messages.js (ARCHITECTURE §9,
// §10, L2c). No React, no ticking timers — every value here is a static snapshot computed at
// call time. avatar_url / time_string / time_ago_string keep the original rounding + locale
// quirks; recalc_image_scale keeps the pixel-avatar DPR math; censorSpoilerText keeps the
// exact spoiler regex (including the empty-string result when the optional group is absent).
import type { Author, Id, User } from '../data/types'
...
if (window.devicePixelRatio > 1) AVATAR_SIZE = 100
export { AVATAR_SIZE }
...
export function avatar_url(
  user: User | Author | null | undefined,
  size: number = AVATAR_SIZE,
): string {
  if (!user || !user.avatar || user.avatar === '0') return 'resource/avatar.png'
  ...
  if (Object.prototype.hasOwnProperty.call(user, 'avatar_pixel') && (user as Author).avatar_pixel)
    return Req.image_url(user.avatar as unknown as Id)
  else return Req.image_url(user.avatar as unknown as Id, size, true)
}
...
export function time_ago_string(date: Date | null | undefined): string {
  if (!date) return 'When?'
  let t = date.getTime()
...
export function censorSpoilerText(text: string): string {
  if (''.startsWith.call(text, '\\h')) text = text.replace(/^\\h(\[.*?\])?[^]*/, '<spoiler $1>')
  return text
}
...
export function recalc_image_scale(img: HTMLImageElement): void {
