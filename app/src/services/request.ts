// REST transport ported near-verbatim from src/request.js (ARCHITECTURE §10, L3a). `ApiRequest`
// is an XMLHttpRequest subclass that fires in its constructor, gates responseType=json on the
// Content-Type header, runs the exact status/retry table (502/408/524/429 with Retry-After and NO
// cap), and on failure invokes the completion callback as cb(SELF_DESTRUCT(err), err) BEFORE
// throwing. `Req` is the sealed singleton facade owning auth-token storage and request builders.
import type { ApiDone, BaseEntity, Chain, Id, User } from '../data/types'
...
// --- Sidebar bridge (log_out → Sidebar.tabs.select('user')) --------------------------------------
// The old `log_out` reached into the global `Sidebar.tabs.select('user')`. Sidebar is an L6
// component that does not exist at this layer, so it registers its tab-select handler here at mount
// (mirrors the L2d sidebar-log setPrintSink pattern). Until registered the call is a no-op; in the
// assembled app the Sidebar registers it before any logout is possible.
type TabSelect = (name: string) => void
let sidebarTabSelect: TabSelect | null = null
export function setSidebarTabSelect(fn: TabSelect | null): void {
  sidebarTabSelect = fn
}
...
class InvalidRequestError extends TypeError {
  ...
  override get message(): string {
    if ('string' == typeof this.resp) return '\n' + this.resp
    const lines = ['']
    if (!this.resp) lines.push('???')
    if (this.resp.title) lines.push(this.resp.title)
    if (this.resp.errors)
      for (const key in this.resp.errors) {
        const msg = this.resp.errors[key] as string[]
        lines.push(`❌${key}:`)
...
export class ApiRequest extends XMLHttpRequest {
  ...
  go(): void {
    ...
    if (Req.auth) this.setRequestHeader('AUTHORIZATION', 'Bearer ' + Req.auth)
    this.send(this.body)
  }
  override abort(): void {
    super.abort()
  ...
  retry(time: number, reason: string): void {
  ...
  override onreadystatechange = (): void => {
    if (this.aborted) return

    switch (this.readyState) {
      case XMLHttpRequest.HEADERS_RECEIVED: {
        const type = this.getResponseHeader('Content-Type')
        if (/[/+]json(;| |$)/i.test(type!)) this.responseType = 'json'
        return
      }
      case XMLHttpRequest.DONE: {
        ...
        switch (this.status) {
          ...
          case 200:
          case 204:
            if (this.proc) resp = this.proc(resp)
            return this.ok(resp)
          // === Invalid request ===
          case 400:
          case 415:
...
export interface ReqApi {
  ...
  uid: number | null
  me: User | null
  request(url: string, proc: ((resp: any) => any) | null, data?: unknown): ApiRequest
  chain(data: Chain, cb?: (resp: any) => void): ApiRequest
  get_auth(username: string, password: string, long?: boolean): ApiRequest
  log_out(): void
  try_load_auth(): boolean
  save_auth(token: string): void
  image_url(id: Id, size?: number, crop?: boolean): string
  delete(type: string, id: Id): ApiRequest
  write(obj: BaseEntity): ApiRequest
  set_watch(pid: Id, state?: boolean): ApiRequest
  send_message(message: unknown): ApiRequest
  send_module_message(name: string, pid: Id, content: unknown): ApiRequest
  search_modules(): ApiRequest
  upload_file(file: File, params: Record<string, any>): ApiRequest
}

export const Req: ReqApi = {
  ...
  request(url, proc, data) {
    let method = 'GET'
    let body: XMLHttpRequestBodyInit | null | undefined = data as XMLHttpRequestBodyInit | null | undefined
    if (data !== undefined) {
      method = 'POST'
      if (data != null) body = to_blob(data)
    }
  ...
  get_auth(username, password, long) {
    const s: { username: string; password: string; expireSeconds?: number } = { username, password }
    if (long) s.expireSeconds = 1 * 60 * 60 * 24 * 29.53 * 3
  ...
  try_load_auth() {
    try {
      ...
      if (!this.auth) throw 'no auth token in localstorage'
      const data = JSON.parse(window.atob(this.auth.split('.')[1])) //yeah
      this.uid = +data.uid
      if (!this.uid) throw "couldn't find uid"
      //let expire = data.exp
  ...
  image_url(id, size, crop) {
    ...
    if (size) {
      url += `?size=${size}`
      if (crop) url += `&crop=true`
    }
    return url
  },

  delete(type, id) {
    if (!TYPES[type]) throw new TypeError('Tried to delete unknown entity type: ' + type)
    return new ApiRequest(`Delete/${type}/${id}`, 'POST', null, TYPES[type])
  },
  write(obj) {
    const type = obj.Type
    if (!TYPES[type]) throw new TypeError('Tried to write unknown entity type: ' + type)
  ...
  upload_file(file, params) {
    ...
    function set(name: string, value: any): void {
      if (value === '') console.warn(`form[‘${name}’]: empty string will be treated as null`)
      form.set(name, value)
    }
    ...
    for (const name in params) {
      ...
      if (name == 'values') {
        for (const key in value) set(`values[${key}]`, value[key])
      } else {
        if (name == 'globalPerms' && value === '') value = '.'
        set(name, value)
      }
    }
    //console.log(form)
