// The chat MessageList imperative controller (messages.js → ARCHITECTURE §8.2/§10, L4b).
// Ported VERBATIM: a circular doubly-linked-list whose sentinel IS the MessageList instance
// (`this.next`/`this.prev`), a `parts` Map keyed by message id, `check_merge` (same merge_hash
// AND within 5 minutes), positional `draw_block`/`draw_part` DOM building, block splice on
// insert / merge-heal on remove, `rethread`, `load_messages_near` with boundary-top/bottom
// markers, the single shared <message-controls> element moved across lists, the `message_control`
// CustomEvent + capture-phase `detail.data` injection, the %uid%→username in-place `msg.text`
// substitution, and the tri-state `display_live` return contract (true|false|null).
//
// PRESERVED BUGS / QUIRKS (do NOT "fix"):
//   - over_limit()/limit_messages() read `this.parts.length` on a Map (no `.length` → undefined),
//     so the 500-message limit is permanently inert. Reproduced via a cast, not repaired.
//   - the `cancellable` (sic) CustomEventInit key — a typo the platform ignores — is kept.
//   - get_reply_message resolves AFTER reject on error (touches the poison resp), kept verbatim.
//   - View.observer forwarded as `intersection_observer` is ALWAYS null (view.js never assigns it;
//     only Draw.observer is toggled), so we pass null — runtime-identical, not "fixed".
//
// React never renders a message; the L? island instantiates this controller and feeds it events.
// Markup is rendered through the markup.ts `convert()` wrapper (== Markup.convert_lang).
...
// A rendered part node in the linked list; opaque to callers (frozen L0 contract = unknown).
export type MessagePart = unknown

// Internal linked-list node shapes. The list is circular: the MessageList instance is the
// sentinel, so a node is either a real `Part` or the `MessageList` itself.
interface Part {
  data: Message
  elem: HTMLElement
  prev: ListNode
...
type ListNode = Part | MessageList

interface MessageControlDetail {
  data: unknown
  action: string
}
...
function tmpl_part(): HTMLElement {
...
function tmpl_reply(): HTMLElement {
...
function tmpl_block(): HTMLElement {
...
function tmpl_nickname(): DocumentFragment {
...
function tmpl_bridge(): DocumentFragment {
  const f = document.createDocumentFragment()
  f.append(' ')
  const i = document.createElement('i')
...
function tmpl_module(): DocumentFragment {
  const f = document.createDocumentFragment()
  f.append(' ')
  const i = document.createElement('i')
  const img = document.createElement('img')
...
function tmpl_avatar(): HTMLImageElement {
  const img = document.createElement('img')
...
function tmpl_module_name(): HTMLSpanElement {
  const s = document.createElement('span')
...
export class MessageList {
  ...
  remove(part: MessagePart): void {
    if (part == this) throw new TypeError('tried to remove list terminator')
    ...
    next.prev = prev
    // remove element
    if (elem == MessageList.controls_message) MessageList.show_controls(null)
    if (elem.nextSibling || elem.previousSibling) {
      elem.remove()
    } else {
      // remove the message block
      ;(elem.parentNode!.parentNode as HTMLElement).remove()
      // was first or last message block
      if (next == this || prev == this) return
      // merge surrounding blocks, if needed
      if (this.check_merge((prev as Part).data, (next as Part).data)) {
        const die = (next as Part).elem.parentNode as HTMLElement
        ;((prev as Part).elem.parentNode as Element).append(...die.childNodes)
        ;(die.parentNode as HTMLElement).remove()
      }
    }
  }
  ...
  draw_reply_block(target: HTMLElement, msg?: Message): void {
  ...
  // msg: Message
  // return: Element
  draw_part(msg: Message): HTMLElement {
    const e = tmpl_part()
    e.dataset.id = String(msg.id)
    if (msg.edited) e.className += ' edited'
  ...
  add_part(msg: Message, prev: ListNode, next: ListNode): Part {
    const elem = this.draw_part(msg)
    const part: Part = { data: msg, elem, prev, next }
  ...
  display_top(msg: Message): MessagePart {
    ...
    if (this.check_merge(part.data, (next as Part).data))
      (next as Part).elem.before(part.elem) // todo: timestamp
    else this.$list.prepend(MessageList.draw_block(msg, part.elem))
    return part
  }
  ...
  display_bottom(msg: Message): MessagePart {
    ...
    if (this.check_merge((prev as Part).data, part.data)) (prev as Part).elem.after(part.elem)
    else this.$list.append(MessageList.draw_block(msg, part.elem))
    return part
  }
  ...
  display_around(relative: MessagePart, msg: Message, where: 'before' | 'after' = 'before'): MessagePart {
    ...
    const same_block = (prev as Part).elem.parentNode == (next as Part).elem.parentNode

    const part = this.add_part(msg, prev, next)
    if (this.check_merge(msg, (next as Part).data)) {
      // print('new message part can be part of next message block')
      ;(next as Part).elem.before(part.elem)
      return part
    }

    if (this.check_merge((prev as Part).data, msg)) {
      // print('new message part can be part of prev message block')
      ;(prev as Part).elem.after(part.elem)
      return part
    }

    const next_block = (next as Part).elem.parentNode!.parentNode as HTMLElement
    ...
    if (!same_block) {
      ...
      const parts = Array.from(((next as Part).elem.parentNode as ParentNode).children)
      const next_index = parts.indexOf((next as Part).elem)
      next_block.after(block_containing_msg)
      const splice_block = MessageList.draw_block((next as Part).data)
      block_containing_msg.after(splice_block)
      ;(Element.prototype.append as (...n: (Node | string)[]) => void).apply(
        splice_block.lastChild as Element,
        parts.splice(next_index),
      )
    }
  ...
  replace(existing: MessagePart, msg: Message): MessagePart {
    if (existing == this) throw new TypeError('tried to replace list terminator')
    ...
    if (!msg.edited) print('warning: duplicate message ', id)
  ...
  // null - nothing
  display_live(msg: Message, cb: (() => void) | null = null): boolean | null {
    const id = msg.id
    ...
    // old message
    if (id < (this.next as Part).data.id) return null
  ...
  display_edge(msg: Message): MessagePart {
    ...
    if (this.next == this) return this.display_only(msg)
    if (id > (this.prev as Part).data.id) return this.display_bottom(msg)
    if (id < (this.next as Part).data.id) return this.display_top(msg)
    ...
    Lp.chain(
      ...
      (resp) => {
        ...
        for (const c of resp.message) {
          ...
          if (part && first) {
            ;(part as Part).elem.classList.add('boundary-' + (top ? 'bottom' : 'top'))
            first = false
          }
        }
  ...
  over_limit(): boolean {
    return (this.parts as unknown as { length: number }).length > this.max_parts
  }
  limit_messages(): void {
    const over = (this.parts as unknown as { length: number }).length - this.max_parts
    for (let i = 0; i < over; i++) this.remove(this.next)
  }

  rethread(msg: Message): MessagePart {
    let pivot = this.prev
    while (pivot != this && (pivot as Part).data.id > msg.id) pivot = (pivot as Part).prev
    return this.display_around(pivot, msg, 'after')
  }

  // ---- shared floating controls (statics) --------------------------------------------

  static controls: HTMLElement = document.createElement('message-controls')
  static controls_message: Element | null = null
  static control_buttons: Record<string, HTMLButtonElement> = Object.create(null)

  // elem: <message-part> or null
  static show_controls(elem: Element | null): void {
    if (elem == this.controls_message) return // shouldn't happen?
    if (elem) {
      elem.before(this.controls)
    } else this.controls.remove()
    ...
    if (elem) {
      ...
      this.control_buttons.reply.hidden = me as boolean
      this.control_buttons.edit.hidden = !me
    }
  }

  static send_mce(action: string, data: unknown, target: EventTarget): void {
    const ev2 = new CustomEvent<MessageControlDetail>('message_control', {
      bubbles: true,
      cancellable: true,
      detail: { data, action },
    } as CustomEventInit<MessageControlDetail>)
  ...
  static init(): void {
    ...
    const btn = (action: string, label: string) => {
      ...
      b.onclick = () => {
        this.send_mce(action, null, this.controls_message as EventTarget)
      }
    ...
    const listen = (ev: string, fn: (e: Event) => void) => {
    ...
    const enter = (ev: Event) => {
      ...
      if (!elem || elem.classList.contains('message-list')) this.show_controls(null)
      else if (elem.tagName == 'MESSAGE-PART') this.show_controls(elem)
    ...
    if (IOS_SAFARI) {
      ...
      listen('touchstart', (ev) => {
        if (!this.controls_message) return
        if (this.controls.contains(ev.target as Node)) return
        if (this.controls_message.contains(ev.target as Node)) {
          //return
        }
  ...
  static draw_block(comment: Message, part?: HTMLElement): HTMLElement {
    const e = tmpl_block()
    ...
    const header = e.firstChild!.nextSibling as HTMLElement

    const name = header.firstChild as HTMLElement
    const name_first = name.firstChild as HTMLElement
    if (module !== null) {
      name_first.textContent = module
      name_first.classList.add('module-name')
      const module_elem = tmpl_module()
      const module_avatar = (module_elem.lastChild as HTMLElement).firstElementChild as HTMLImageElement
      const module_user = (module_elem.lastChild as HTMLElement).lastElementChild as HTMLElement
      ...
      if (author.bridge) name.appendChild(tmpl_bridge())
      else {
        const nickname = tmpl_nickname()
        const realname = (nickname.lastChild as HTMLElement).lastElementChild as HTMLElement
        realname.textContent = author.username
        name.appendChild(nickname)
      }
    }

    const time = header.lastChild as HTMLElement
...
document.addEventListener('message_control', (ev) => {
  const e = ev as CustomEvent<MessageControlDetail>
  if (e.detail.action == 'info' || e.detail.action == 'raw')
    alert(JSON.stringify(e.detail.data, null, 1)) // <small heart>
  if (e.detail.action == 'link') {
    navigator.clipboard.writeText(`sbs:comments?ids=${(e.detail.data as Message).id}`)
    print('copied link')
  }
})
