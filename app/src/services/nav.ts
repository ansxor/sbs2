...
//
// Cross-layer effects the old ViewSlot triggered inline are injected as sinks so this module
// depends only on L0 types (matches the setPrintSink/setImageLoader precedents): the sidebar
// "close on same-slot navigation" and the active-editor accessor `view()`.
import type { BaseEntity, NavLocation, SlotDescriptor } from '../data/types'
import { handle_redirect } from '../routing/view-registry'

export interface NavApi {
  slots: SlotDescriptor[]
  focused: SlotDescriptor | null
  parse_url(source: string): NavLocation
  unparse_url(location: NavLocation): string
  serialize(): string
  make_url(): string
  entity_link(entity: BaseEntity): string
  start(): void
  set_address(replace?: boolean): void
  focused_slot(): SlotDescriptor
  set_focus(slot: SlotDescriptor): void
  load_location(location: NavLocation, slot?: SlotDescriptor): void
  update_from_fragment(fragment: string): void
  // external store contract for React (useNavSlots)
  subscribe(cb: () => void): () => void
  getSlots(): SlotDescriptor[]
  view(): unknown
}
...
// (navigate.js:460).
export function setCloseFullscreen(fn: () => void): void {
  closeFullscreen = fn
}
...
function slot_from_el(el: Element): SlotDescriptor | null {
...
function load_slot_url(slot: SlotDescriptor, rawUrl: string): boolean {
  if (slot.url === rawUrl) return false
...
function load_slot_location(slot: SlotDescriptor, location: NavLocation): boolean {
  const old = slot.url
  const canonical = handle_redirect(location)
  slot.url = Nav.unparse_url(canonical)
  Nav.set_address()
  if (slot.url === old) return false
  notify()
  return true
}
...
export const Nav: NavApi = {
  ...
  focused_slot() {
    if (Nav.focused) return Nav.focused
    if (!slots[0]) slots.push(create_slot())
    return slots[0]!
  },

  set_focus(slot: SlotDescriptor) {
    if (Nav.focused === slot) return
    Nav.focused = slot
    notify()
  },
  ...
  entity_link(entity: BaseEntity): string {
    const type = (
      { user: 'user', content: 'page', category: 'category' } as Record<string, string>
    )[entity.Type]
    if (!type) throw new Error('idk entity type')
    return '#' + type + '/' + (entity as { id?: unknown }).id
  },

  // navigate.js:314 — parse a single slot-url string into {type,id,query,fragment}.
  parse_url(source: string): NavLocation {
    const [, typeRaw, idRaw, query_str, fragment] =
      /^(.*?)(?:[/](.*?))?([?&].*?)?(?:[#](.*))?$/.exec(source)!
    ...
    if (idRaw == undefined) id = null
      ...
      else if (decoded[0] == '@') id = decoded.substring(1)
      else id = decoded
    }
  ...
  unparse_url(location: NavLocation): string {
    function esc(str: string, regex?: RegExp): string {
      // allow: \w - . ! * ' $ + , : ; @
      str = encodeURI(str).replace(/[)~(]+/g, escape)
      if (regex) str = str.replace(regex, encodeURIComponent)
      return str
    }
    ...
    if (location.fragment != null) url += '#' + esc(location.fragment)
  ...
  set_address(replace = false): void {
  ...
  update_from_fragment(fragment: string): void {
    ...
    if (Nav.focused && !slots.includes(Nav.focused)) Nav.focused = slots[0] ?? null
...
document.addEventListener(
  ...
  (ev) => {
    const el = ev.target
    const link = (el instanceof Element ? el.closest(':any-link') : null) as
      | HTMLAnchorElement
      | null
    if (!link) return
    ...
    if (target != 'self') {
      ...
      if (!load_slot_url(slot, href.slice(1))) closeFullscreen()
    } else {
      // #anchor url
      const root: ParentNode = slotEl ?? document
      const anchor = root.querySelector(`a[name="${CSS.escape(href.slice(1))}"]`)
      if (anchor) (anchor as HTMLElement).scrollIntoView()
    }
  },
  { capture: true },
)
