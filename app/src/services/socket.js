...
let statusUpdateUser = null;
export function setStatusUpdateUser(fn) {
    statusUpdateUser = fn;
}
...
export const Lp = {
    ...
    handler_count() {
        ...
        if (num)
            document.documentElement.dataset.socketPending = String(num);
        else
    ...
    on_ready() {
        ...
        for (const { request } of this.handlers.values())
            this.send(request);
        this.ready = true;
    },
    is_alive() {
