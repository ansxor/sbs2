...
import type { BusCategory, IdListeners } from '../services/events'
...
export function useSetting<T>(name: string): T {
  return useSyncExternalStore(Settings.subscribe, () => Settings.get(name) as T)
}
...
export function useBusListener(
  cat: BusCategory,
  cb: (...a: any[]) => void,
  opts?: { id?: number },
): void {
  ...
  useEffect(() => {
    ...
    if (id !== undefined) (bucket as IdListeners).listen_id(view, id, wrapper)
    else bucket.listen(view, wrapper)
    return () => {
...
export function useNavSlots(): SlotDescriptor[] {
...
function subscribeDocEl(onChange: () => void): () => void {
...
let loginSnapshot: { loggedIn: boolean; uid: number } = { loggedIn: false, uid: 0 }
function getLoginSnapshot(): { loggedIn: boolean; uid: number } {
...
let socketSnapshot: { state: string; pending: boolean } = { state: '', pending: false }
function getSocketSnapshot(): { state: string; pending: boolean } {
...
export function useLoginState(): { loggedIn: boolean; uid: number } {
...
export function useSocketState(): { state: string; pending: boolean } {
